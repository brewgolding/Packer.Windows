build {
  ...
  source "hyperv-vmcx.hyperv-vmcx.Base-EFI-VMCX" {
    ...
    clone_from_vmcx_path = "${var.build.directory}/${var.windows_11_pro.name}"
    output_directory = "${var.build.directory}/${var.windows_11_pro.name}.DSCProvision"
    vm_name = "packer-${var.windows_11_pro.name}.DSCProvision"
  }

  source "hyperv-vmcx.hyperv-vmcx.Base-EFI-VMCX" {
    name = "Windows-Server-2022-Datacenter-DSC"
    clone_from_vmcx_path = "${var.build.directory}/${var.server_2022.name}"
    output_directory = "${var.build.directory}/${var.server_2022.name}.DSCProvision"
